...
from utils.finetune_utils import make_finetune_dataloaders, freeze_bert_layers, \
...
function_names = ["variable_angle", "variable_ballheight", "variable_ballheight_angle", "variable_shelfheight",
                  "variable_shelfheight_angle", "variable_shelfheight_ballheight",
...
for train_folder, test_folder in zip(train_folders, test_folders):
    train_data = [pd.read_csv(folder_loc + folder + '/train_data.csv') for folder in train_folder]
    val_data = [pd.read_csv(folder_loc + folder + '/val_data.csv') for folder in train_folder]
    ...
    train_dataloader, val_dataloader, test_dataloader = make_finetune_dataloaders(train_data=train_data,
                                                                                  val_data=val_data,
                                                                                  test_data=test_data,
                                                                                  tokenizer=tokenizer,
    ...
    try:
        ...
        batch_logger.log_info(f"{test_folder} data with {output_type} output, {len(train_data)} training examples "
