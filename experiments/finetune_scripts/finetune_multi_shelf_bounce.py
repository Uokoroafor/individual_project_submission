...
# Create the logger
batch_logger = TrainingLogger("msb_finetune_logs_num.txt", verbose=False)
...
folder_loc = "data/multi_shelf_bounce/"
function_names = [
    "variable_angle",
    "variable_time",
...
for function_name in function_names:
    ...
    oos_dataloader = make_finetune_dataloader(
        data=oos_test_data,
        tokenizer=tokenizer,
        max_length=max_length,
        batch_size=batch_size,
    ...
    try:
        ...
        oos_test_loss = BertTrainer.log_numerical_outputs(
            oos_dataloader, output_type=output_type, oos_str="oos_"
        )

        batch_logger.log_info(
            f"{function_name} data with {output_type} output, {len(train_data)} training examples "
