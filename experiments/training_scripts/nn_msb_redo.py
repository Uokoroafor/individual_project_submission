...
training_hyperparams = load_config("../../config_files/config_structure_nn.txt")
...
for folder in folders:
    ...
    data_amounts = [200_000]

    num_input_features = 14
    hidden_layers = [528, 528, 256]
    ...
    for data_amount in data_amounts:
        if not stop_training:
            ...
            oos_test_data = PhysicalDataset(
                pd.read_csv(data_folder + oos_test_data_path)
            )

            print(f"Size of train dataset: {len(train_data):,}")
            if len(train_data) < data_amount and not stop_training:
                print(
                    f"WARNING: Only {len(train_data):,} training examples using all data"
                )
                stop_training = True
